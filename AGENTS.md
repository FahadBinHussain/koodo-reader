...
- If a cloud book binary still exists but a stale local `delete` record hides its metadata, sync now removes that stale delete during reconciliation and restores the cloud row. A missing binary is never resurrected from metadata alone.
